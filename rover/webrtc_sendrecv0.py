...
PIPELINE_DESC = '''
webrtcbin name=sendrecv bundle-policy=max-bundle stun-server=stun://stun.l.google.com:19302
 v4l2src device=/dev/video0 ! image/jpeg,framerate=30/1,width=640,height=480 ! jpegdec ! videoconvert ! queue ! vp8enc deadline=2 ! rtpvp8pay !
 queue ! application/x-rtp,media=video,encoding-name=VP8,payload=97 ! sendrecv.
'''
...
ap_sock.bind(("0.0.0.0", 0))
AP_PORT = ('127.0.0.1', 22334)
    ...
    async def loop(self):
        ...
        async for message in self.conn:
            if message == 'HELLO':
                ...
                # vehicle.mode = 'MANUAL'
                current_mode = 'MANUAL'
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
                ...
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('TURNLEFT45'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('TURNLEFT90'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('TURNRIGHT45'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('TURNRIGHT90'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('TURN180'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
            elif message.startswith('ARMDISARM'):
                print(message)
                pkt = pickle.dumps(message)
                ap_sock.sendto(pkt, AP_PORT)
